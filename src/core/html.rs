...
pub fn slice_between_ci<'a>(s: &'a str, open_pat: &str, close_pat: &str) -> Option<&'a str> {
    let lc = to_lower(s); let open = to_lower(open_pat); let close = to_lower(close_pat);
    let o = lc.find(&open)?; let after = s[o..].find('>')? + o + 1;
    let cr = lc[after..].find(&close)?; Some(&s[after..after+cr])
}
pub fn next_tag_block_ci(s: &str, o: &str, c: &str, from: usize) -> Option<(usize, usize)> {
    let lc = to_lower(s); let ol = to_lower(o); let cl = to_lower(c);
    let start = lc.get(from..)?.find(&ol)? + from; let open_end = s[start..].find('>')? + start + 1;
    let end_rel = lc[open_end..].find(&cl)?; let end = open_end + end_rel + c.len(); Some((start, end))
}
...
pub fn strip_tags(s: String) -> String {
    ...
    for ch in s.chars(){ match ch{ '<'=>in_tag=true,'>'=>in_tag=false,_ if !in_tag=>out.push(ch),_=>{} } }
